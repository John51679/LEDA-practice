...
int main(){
	srand(time(0)); /*I tried to generalize the whole graph familly (as well as the Levels of "sub-graphs") and so i had to use "rand" function a lot. So i'm also using srand for the sequence*/
    graph G;
	
	for(int i=0; i<NODES*LEVELS; i++){ //First initialize all nodes
		G.new_node();
	}
	list<node> n = G.all_nodes(); /*Keep a list of all nodes because that way we can specify a range for "rand" function's "random" number generator (which will technically work as a random node generator)*/
	list<node> chosenNodes; /*Keep a list of randomly chosen nodes (one node per level) */
	node v;
	for(int i=0; i<LEVELS; i++){ /*Pick a node randomly for each level and add it to list chosenNodes*/
		v = n.contents(n.get_item((int)(rand()%NODES + i*NODES)));
		chosenNodes.append(v);
	}

	/*This for is used to make the rest of the graph according to the explanation given at my report*/
	node vt1,vt2,vt3,vt4;
	for(int i = 0; i<NODES; i++){
		for(int j = 1; j < LEVELS; j++){
			vt1 = n.contents(n.get_item((j-1)*NODES + i));
			vt2 = n.contents(n.get_item(j*NODES + i));
			if (vt1 == chosenNodes.contents(chosenNodes.get_item(j-1))){
				G.new_edge(chosenNodes.contents(chosenNodes.get_item(j)),vt2);
			}
			else if (vt2 == chosenNodes.contents(chosenNodes.get_item(j))){
				G.new_edge(chosenNodes.contents(chosenNodes.get_item(j-1)),vt1);
			}
			else G.new_edge(vt1,vt2);
			if (((j-1) + 2) < LEVELS){
				vt3 = chosenNodes.contents(chosenNodes.get_item(j-1));
				vt4 = n.contents(n.get_item(((j-1) + 2)*NODES + i));
				if(vt4 != chosenNodes.contents(chosenNodes.get_item((j-1) + 2))) G.new_edge(vt3,vt4);
			}
		}
	}
